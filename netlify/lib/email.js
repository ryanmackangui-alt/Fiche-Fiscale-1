...
export function emailHtml(fiche, urlSite, dateStr, date = new Date()) {
  if (VOIX.sobre) return emailSobre(fiche, urlSite, dateStr, date);
...
------------------------------------------------------------------ */
function emailSobre(fiche, urlSite, dateStr, date) {
  const paragraphes = introDuJour(date).map((t) => t.replace("{salutation}", salutation(date)));
